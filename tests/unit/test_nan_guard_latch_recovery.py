...
@pytest.mark.parametrize("is_sdxl", [False, True], ids=["sd15", "sdxl-trt"])
def test_delayed_bad_verdict_does_not_freeze_attention_history(is_sdxl):
    """A delayed model verdict must not discard clean cache replacement frames."""
    from streamdiffusion.utils.nan_guard import NanGuard

    class DelayedBadGuard(NanGuard):
        def sanitize_(self, tensor):
            super().sanitize_(tensor)
            # CUDA may retain a bad verdict over several queued clean calls.
            return True

    stream = _make_stream([22, 36], _NanInjectingKvoUnet(nan_on_call=99))
    stream.is_sdxl = is_sdxl
    stream._is_unet_tensorrt = is_sdxl
    stream._lora_scale_tensor = None
    stream._nan_guard_model_pred = DelayedBadGuard("delayed model verdict")
    _wire_bucketed_kvo_cache(stream)
    stream._kvo_buckets[0].fill_(float("nan"))
    for _ in range(stream.cache_maxframes):
        stream.predict_x0_batch(_zero_input(stream))
    assert stream.frame_idx == stream.cache_maxframes
    assert torch.isfinite(stream._kvo_buckets[0]).all()
