...
def test_new_export_uses_shared_versioned_cache_without_touching_old_engine(tmp_path):
    old = tmp_path / 'hed.engine'
    old.write_bytes(b'original-engine')
    paths = []
    for cls in (HEDTensorrtPreprocessor, ScribbleTensorrtPreprocessor):
        proc = cls.__new__(cls)
        proc.params = {'engine_path': str(old)}
        paths.append(proc._get_engine_path())
        assert proc._get_engine_path() != old
        proc.params['engine_path'] = str(paths[-1])
        assert proc._get_engine_path() == paths[-1]
    assert paths[0] == paths[1]
    assert old.read_bytes() == b'original-engine'
