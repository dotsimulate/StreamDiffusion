...
class TestUnnormalizedTwoPromptSlerp:
    def test_zero_weight_third_prompt_does_not_change_raw_weight_blend(self):
        updater = _make_updater()
        updater.normalize_prompt_weights = False
        updater._prompt_cache = {
            name: {"embed": _rand_embed(seed=seed)}
            for name, seed in (("cat", 401), ("dog", 402), ("unused", 403))
        }
        for total in (0.5, 1.0, 2.0):
            updater._current_prompt_list = [("cat", 0.25 * total), ("dog", 0.75 * total)]
            updater._apply_prompt_blending("slerp")
            two_prompts = updater.stream.prompt_embeds.clone()
            updater._current_prompt_list.append(("unused", 0.0))
            updater._apply_prompt_blending("slerp")
            torch.testing.assert_close(two_prompts, updater.stream.prompt_embeds)
